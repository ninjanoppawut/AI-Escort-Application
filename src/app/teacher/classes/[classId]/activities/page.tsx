import { notFound, redirect } from "next/navigation";
import { z } from "zod";

import { TeacherActivityList } from "@/features/activities/components/activity-list";
import type { ActivityUiErrorCode } from "@/features/activities/errors";
import { listClassActivities } from "@/features/activities/server/operations";
import { getActiveIdentity } from "@/features/auth/server/identity";
import { createSupabaseServerClient } from "@/lib/supabase/server";

export const dynamic = "force-dynamic";

export default async function TeacherClassActivitiesPage({
  params,
}: {
  params: Promise<{ classId: string }>;
}) {
  const { classId } = await params;
  if (!z.uuid().safeParse(classId).success) notFound();

  const identity = await getActiveIdentity();
  if (identity.error) {
    if (
      identity.error === "AUTH_REQUIRED" ||
      identity.error === "EMAIL_NOT_CONFIRMED"
    ) {
      const query = new URLSearchParams({
        error: identity.error,
        returnTo: `/teacher/classes/${classId}/activities`,
      });
      redirect(`/auth/sign-in?${query.toString()}`);
    }
    redirect(`/auth/error?code=${identity.error}`);
  }

  if (identity.identity.account_type !== "teacher") {
    return (
      <TeacherActivityList
        classId={classId}
        initialErrorCode="FORBIDDEN"
        initialList={null}
      />
    );
  }

  const result = await listClassActivities(
    await createSupabaseServerClient(),
    classId,
  );
  const list = result.data?.viewerRole === "teacher" ? result.data : null;

  return (
    <TeacherActivityList
      classId={classId}
      initialErrorCode={
        result.error
          ? (result.error.code as ActivityUiErrorCode)
          : list
            ? null
            : "FORBIDDEN"
      }
      initialList={list}
    />
  );
}
