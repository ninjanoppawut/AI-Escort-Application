...
import { deleteOrArchiveGroup } from "@/features/groups/server/lifecycle";
...
// Deletes an unused group or archives a group with session history; the
// database decides which, so there is no client-selected archive flag.
export async function DELETE(
  ...
  const result = await deleteOrArchiveGroup(
