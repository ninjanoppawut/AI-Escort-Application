...
### D-071 — Admin console defaults

- **Status:** accepted as an implementation default (P15-01..P15-05, P15-07)
- **Decision:** Every admin page and RPC requires an active `platform_admins` grant and an `aal2` session, checked in the database; a grant without `aal2` goes to a TOTP enroll/verify step. Every successful admin view, list, and change writes an audit row; search text and free-text reasons are not copied into audit payloads (only that they were used or their length). The directory shows teacher emails (needed for provisioning support) and masks student emails (`a***@domain`). Active school names are unique regardless of case, and archiving a school revokes its pending teacher invitations. A teacher invitation's raw token is shown once to the issuing admin as a `/teacher-invite/[token]` link and is never listed again. Operational error events carry flow/stage/code, correlation IDs, and only allowlisted numeric or category context, capped at 60 per caller per 10 minutes. Flow health reports AI, Realtime, and request volume/latency as partial telemetry rather than healthy. Incidents are acknowledged idempotently, notes are append-only with a client note ID for safe retry, and closing needs a written resolution.
- **Consequence:** Hosted projects must enable TOTP MFA in Auth settings before the admin console can be used. Break-glass (P15-06) is not built until approved for the deployment.
